...
def deploy_contract():
    ...
    CONTRACT_ADDRESS = server.toChecksumAddress(content[45].split(': ')[1].strip())
...
def fetch_file_data():
    ...
    for i in users:
        ipfsHash = contract.functions.getIpfsHashForUser(i).call()
        fileName = contract.functions.getFileNameForUser(i).call()
        print(fileName)
        model_number = contract.functions.getModelNumberForUser(i).call()
